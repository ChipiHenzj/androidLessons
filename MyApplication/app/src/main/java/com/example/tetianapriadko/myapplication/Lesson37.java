...
public class Lesson37 extends AppCompatActivity {

    final String LOG_TAG = "myLogs";

    // Table position
    int[] position_id = { 1, 2, 3, 4 };
    String[] position_name = { "Boss", "Programmer", "Accountant", "Security guard" };
    int[] position_salary = { 15000, 13000, 10000, 8000 };

    // Table people
    String[] people_name = { "Kristaps", "Agnese", "Rolands", "Armands", "Inga", "Maris", "Lauris", "Martins" };
    ...
    void logCursor(Cursor cursor) {
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                ...
                do {
                    string = "";
                    for (String string1 : cursor.getColumnNames()) {
                        string = string.concat(string1 + " = "
                                + cursor.getString(cursor.getColumnIndex(string1))
                                + "; ");
                    }
